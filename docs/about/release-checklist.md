...
候选版本为 `wiki-v1.1.2`，上一已知良好版本为 `wiki-v1.0.0`。`wiki-v1.1.0` 与 `wiki-v1.1.1` 保留为未部署成功的历史候选标签，不得移动或覆盖。仓库处于整合阶段时，发布清单必须显示 `WORKTREE / candidate`。只有源代码进入干净提交、不可变新标签指向该提交，并且上线后检查通过，维护人员才能把该版本登记为线上已验证发布。
...
3. 在该提交上创建附注标签 `wiki-v1.1.2`。不得移动或覆盖 `wiki-v1.0.0` 与失败候选标签 `wiki-v1.1.0`、`wiki-v1.1.1`。
4. 重新生成发布清单。
5. 运行严格校验：
