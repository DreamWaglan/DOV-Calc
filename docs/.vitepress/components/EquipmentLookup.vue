<template>
  <section class="equipment-lookup" aria-labelledby="equipment-lookup-title">
    <header class="equipment-lookup__header">
      ...
      <div class="equipment-lookup__stats" role="status" aria-label="装备统计" aria-live="polite" aria-atomic="true">
        <output aria-label="当前结果数量">{{ filteredItems.length }}</output>
        <small>当前结果</small>
      </div>
    </header>

    <div class="equipment-lookup__toolbar" role="search" aria-label="装备筛选">
...
const filteredItems = computed(() => {
  return filterEquipmentItems(data.items, {
    query: query.value,
...
.equipment-lookup__stats output {
  color: var(--c-brand);
  font-size: 28px;
  font-weight: 800;
  line-height: 1;
...
.equipment-lookup input:focus-visible,
.equipment-lookup select:focus-visible {
